...
def get_piece_type(pos: Position, visited: dict, map_: dict):
    reachable = []
    for direction, (dx, dy, opposite) in DIRECTIONS.items():
        new_pos = Position(x=pos.x + dx, y=pos.y + dy)
        if _out_of_map(new_pos, map_) or new_pos not in visited:
            continue
        pipe = map_[new_pos.x][new_pos.y]
        if pipe in PIPES:
            directions = PIPES[pipe]
            if opposite in directions:
                reachable.append(direction)
    for piece, pipe in PIPES.items():
        if len(reachable) == len(pipe):
            if all([x in pipe for x in reachable]):
                return piece
...
def main():
    ...
    print(start)
    visited = walk(start, map_)
    map_[start.x][start.y] = get_piece_type(start, visited, map_)
    for x in range(len(map_)):
        norths = 0
        for y in range(len(map_[x])):
            place = map_[x][y]
            if Position(x, y) in visited:
                directions = PIPES[place]
                if 'n' in directions:
                    norths += 1
                continue
            map_[x][y] = 'O' if norths % 2 == 0 else 'I'
    count = '\n'.join([''.join(line) for line in map_]).count('I')
    print(count)
