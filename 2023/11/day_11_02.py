...
def _enumerate(it, start=0, step=1):
    n = start
    for item in it:
        yield n, item
        n += step
...
def expand(cols: list, rows: list, map_: list, step: int = 1):
    for offset, row in _enumerate(rows, step=step):
        for _ in range(step - 1):
            map_.insert(row + offset, [False for _ in range(len(map_[0]))])
    for offset, col in _enumerate(cols, step=step):
...
def substitute_and_count(map_: list):
    count = 1
    galaxies = {}
    for i, row in enumerate(map_):
        for j, col in enumerate(row):
...
def main():
    ...
    map_ = expand(empty_cols, empty_rows, map_, 1_000_000)
    print(len(map_))
    print(len(map_[0]))
